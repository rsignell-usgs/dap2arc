...
#disable pyproj for writing to EPSG:4326 instead of projecting
pyproj_enabled = False
...
def writeLandXML(nv,x,y,h,xmlTemp):
    ...
    <LandXML version="1.1" date="2008-06-09" time="08:33:4"
     ...
     <Application name="ADCIRC" version="x.x.x" manufacturer="UNC/ND"
      manufacturerURL="http://adcirc.org"><Author createdBy="rsignell@usgs.gov"/></Application>


     <Surfaces>
      <Surface name="Adcirc_Grid">
    '''
...
class Dap2tin(object):
    def __init__(self):
        ...
        self.label = "Dap2tin"
        self.description = "Access data from the ADCIRC Ocean Model via DAP and return a TIN"
        ...
        self.default_urls = ['http://comt.sura.org/thredds/dodsC/data/comt_1_archive/inundation_tropical/UND_ADCIRC/Hurricane_Rita_2D_final_run_with_waves',
    ...
    def getParameterInfo(self):
        ...
        url = arcpy.Parameter(
            ...
            direction="Input")
        # set default value to IOOS testbed Hurricane Ike 3D run
        url.value = 'http://comt.sura.org/thredds/dodsC/data/comt_1_archive/inundation_tropical/UND_ADCIRC/Hurricane_Ike_2D_final_run_with_waves'
        ...
        dataset_var.value = 'zeta'
        dataset_var.filter.type = "ValueList"
        dataset_var.filter.list = ["zeta","depth","windx","windy"]
        ...
        sr.value = arcpy.SpatialReference(4326).factoryCode
    ...
    def execute(self, parameters, messages):
        ...
        elif numdims==1: # node (e.g. water depth 'depth')
